...
function validateForm() {
  ...
  //validating userName .
  if (name == "") {
    /*checking if the mandatory fields are filled*/
    printError("nameErr", "Please fill in your username");
  } else {
    let regex =
      /^[a-z]+\d*$/gi; /*checking if the user has entered  valid data using regular expressions*/
    if (regex.test(name) === false) {
      ...
      printError(
        "nameErr",
        ""
      ); /* this will not return an error if the data entered by a user is valid*/
      nameErr = false;
    }
  }
  //validating password.
  if (password == "") {
    ...
    let regex =
      /[a-zA-Z\d]/gi; /*checking if the user has entered  valid data using regular expressions*/
